...
tree_data <- tree_data %>%
  rename(rad_inc_mm = rad_ib_mm, inc_mm = inc_mm)
...
```{r}
# Create DBH and BA columns
tree_data <- tree_data %>%
  mutate(
    DBH_cm = rad_inc_mm / 10,  # Correct column name
    BA_m2 = 0.00007854 * (DBH_cm^2)  # Basal Area in m2
  )

# Filter for species POTR in 2007 and calculate the mean Basal Area
